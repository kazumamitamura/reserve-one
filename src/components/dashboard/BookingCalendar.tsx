    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/50">
      ...
      <div className="grid grid-cols-7 gap-px bg-slate-200 rounded-lg overflow-hidden">
        ...
        {days.map((day) => {
          ...
          const myBookingCount = daySlots.filter(
            (s) => s.is_booked && s.booked_by === userId
          ).length;
          ...
          return (
            <button
              key={dateKey}
              type="button"
              onClick={() => !isPast && daySlots.length > 0 && setSelectedDate(dateKey)}
              disabled={isPast || daySlots.length === 0}
              className={`min-h-[80px] p-1.5 text-left transition ${
            ...
            >
              ...
              {!isPast && daySlots.length > 0 && (
                <div className="space-y-0.5">
                  {hasAvailable && (
                    <div className="text-xs px-1 py-0.5 rounded bg-green-100 text-green-700 text-center">
                      {availableCount}枠
                    </div>
                  )}
                  {myBookingCount > 0 && (
                    <div className="text-xs px-1 py-0.5 rounded bg-blue-100 text-blue-700 text-center">
                      予約{myBookingCount}
                    </div>
                  )}
                </div>
        ...
        <div className="mt-6 border-t border-slate-200 pt-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-slate-800">
              {format(new Date(selectedDate), "M月d日(E)", { locale: ja })}の枠
            </h3>
            <button
              type="button"
                ...
                return (
                  ...
                  >
                    ...
                    {!slot.is_booked && (
                      ...
                      >
                        {isBooking && bookingSlotId === slot.id ? "予約中..." : "予約"}
                      </button>
                    )}
                  </div>
                );
